...
def quat_dist_alternative(q1,q2=None):
        # import pdb; pdb.set_trace()
        if q2 is None: # means we are computing for evaluation, and should compare to zero unit quaternion
            return(2*np.arccos(abs(q1[...,0])))
...
def rot_dist(q1,q2=None):
        ...
        # for evaluation:
        if (q2 == None): 
                dists = quat_dist_alternative(q1, q2) # use of alternative quaternion distance function.
...
# Joaquin: edited to properly obtain minimum theta indices ('inds')
def fz_reduce(q,syms):

        import pdb; pdb.set_trace()
        shape = q.shape

        q = q.reshape((-1,4)) # reshapes the [x,y,4] tensor into [x*y,4] tensor.

        q_w_syms = outer_prod(q,syms) # adds a dimension to the tensor, to account for 24 symmetric orientations (fcc), making it a [x*y,24,4] tensor.

        dists = rot_dist(q_w_syms)

        inds = dists.min(-1)[1] # this gives you the indices of the minimum orientation (theta) distance. 

        q_fz = q_w_syms[torch.arange(len(q_w_syms)), inds] # selects the orientations with the minimum theta (out of the 24 symmetric equivalencies)
